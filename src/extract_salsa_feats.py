...
# DOWNLOADED ON 18-9-26 FROM https://github.com/itsjunwei/IEEE_Sensors_Enhancing/tree/main
# Python script for calculating a full SALSA-LITE plus CDPD feature set, courtesy of Jun-Wei Yeow et Al. 2025
# Code is modified to fit my (Matthias Winther) specifications
...
# DOA Frequency Bounds (Check the page *** in Notion for deeper explanation)
...
#       - The upper and lower bounds indicate for which bins the contained spatial information is considered valid (due to aliasing)
#       - The most important stft bins are calculated, while the remaining bins up to the limit is compressed via averaging. This is because most of the important cues live in the bins below the cutoff bin.
N_BINS = N_FFT // 2 + 1
LOWER_BIN = max(1, int(FMIN_DOA * N_FFT / FS)) # First stft bin index
UPPER_BIN = int(FMAX_DOA * N_FFT / FS) # End stft bin index
CUTOFF_BIN = int(9000 * N_FFT / FS) # The cutoff bin is determined by a frequency after which information is averaged until the nyquist frequency
...
W = np.zeros((200, N_BINS), dtype=np.float32) # Manually determined 200 bins?
...
# =================================
def get_salsa_dlite(audio_data: np.ndarray, alpha_snr: float = 2.0, lambda_mmsc: float = 0.8, 
    ...
    salsalite_feat = _get_salsalite(audio_data=audio_data) 
    ...
    print(f"Transposed X stft shape, after cutoff: {Y.shape} (Channels, Time, Freq)")

    # Compute average MSC (Magnitude Squared Coherence)
    gamma_avg = compute_msc_recursive(Y, lambda_=lambda_mmsc)  # (Time, Freq)
    print(f"Gamma_avg shape = {gamma_avg.shape} (Time, Freq)")
    ...
    print(f"Shape of padded X: {X.shape} (Frequency, Time, Channels)")
...
if __name__ == "__main__":
    ...
    sample_audio_fp = "data/STARSS23/MIC/mic_dev/dev-train-tau/fold3_room4_mix001.wav"
    ...
    for i in range(3, 8):
        plt.figure()
        plt.pcolormesh(salsadlite_feat[i, :, :])
        plt.savefig(f"test_track_salsa-d-lite_{i}.png")
